...
def main():
    ...
    result_3_rhc = results_to_plot(X_train_array, X_test_array, y_train_hot, y_test_hot, algorithms[1], layers[0])
    result_6_rhc = results_to_plot(X_train_array, X_test_array, y_train_hot, y_test_hot, algorithms[1], layers[1])
    result_9_rhc = results_to_plot(X_train_array, X_test_array, y_train_hot, y_test_hot, algorithms[1], layers[2])
    result_12_rhc = results_to_plot(X_train_array, X_test_array, y_train_hot, y_test_hot, algorithms[1], layers[3])
    ...
    for i in range(1,11,1):
        model = mlrose_hiive.NeuralNetwork(hidden_nodes=[6], activation='relu',
                                        algorithm='simulated_annealing', max_iters= 4200,
                                        bias=True, is_classifier=True, learning_rate=0.1,
                                        early_stopping=True, clip_max=5, max_attempts=100, 
                                        schedule = mlrose_hiive.ArithDecay(init_temp=1.0, decay=(i/10), min_temp=0.01), 
                                        random_state=614)
        model.fit(X_train_array, y_train_hot)
        y_test_pred = model.predict(X_test_array)
        f1score = f1_score(y_test_hot, y_test_pred, average='weighted')
