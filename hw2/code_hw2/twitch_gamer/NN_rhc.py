...
def main():
    ...
    result_3_rhc = results_to_plot(X_train_array, X_test_array, y_train_hot, y_test_hot, algorithms[0], layers[0])
    result_6_rhc = results_to_plot(X_train_array, X_test_array, y_train_hot, y_test_hot, algorithms[0], layers[1])
    result_9_rhc = results_to_plot(X_train_array, X_test_array, y_train_hot, y_test_hot, algorithms[0], layers[2])
    result_12_rhc = results_to_plot(X_train_array, X_test_array, y_train_hot, y_test_hot, algorithms[0], layers[3])
    ...
    for i in range(0,10,1):
        model = mlrose_hiive.NeuralNetwork(hidden_nodes=[6], activation='relu',
                                        algorithm='random_hill_climb', max_iters= 2500,
                                        bias=True, is_classifier=True, learning_rate=0.1,
                                        early_stopping=True, clip_max=5, max_attempts=100, restarts = i,
                                        random_state=614)
        model.fit(X_train_array, y_train_hot)
        y_test_pred = model.predict(X_test_array)
        f1score = f1_score(y_test_hot, y_test_pred, average='weighted')
