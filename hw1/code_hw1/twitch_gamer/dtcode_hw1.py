...
def plot_training_criterion(X_train, y_train, para):
    '''
    This function is to plot training set based on different criterion
    Args:
        X_train --> features in training set which is a dataframe 
        y_train --> target in training set which is a dataframe 
        para --> parameters in decision tree
...
def main():
    ...
    for i in range(1,15):
        ...
        acc_entropy_train.append(balanced_accuracy_score(y_train, pred1))
        acc_entropy_test.append(balanced_accuracy_score(y_test, pred2))
        max_depth.append(i)
